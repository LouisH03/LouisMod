...
static volatile LONG g_native_log_guard = 0;
...
void __cdecl on_button_found(void *button, void *dialog)
{
    ...
    if (button == NULL || g_game_base == NULL) {
        louismod_native_log(
            "native: button capture received invalid state");
        return;
    }

    louismod_native_log("native: replay button capture entered");
    ...
    if (dialog != NULL) {
        ...
        louismod_native_log("native: replay button callback bound");
    }

    button_bytes = (BYTE *)button;
    if (scan_object_for_caption(button_bytes, BUTTON_SCAN_BYTES)) {
        louismod_native_log("native: replay button caption prepared");
        return;
    }
    ...
    for (offset = 0;
        ...
        if (scan_object_for_caption(nested, NESTED_SCAN_BYTES)) {
            louismod_native_log(
                "native: replay button caption prepared in nested control");
            return;
        }
    }

    louismod_native_log(
        "native: replay button caption field was not found");
}
...
static BOOL resolve_powershell_path(WCHAR *path, SIZE_T capacity)
{
    static const WCHAR *const suffixes[] = {
        L"\\SysWOW64\\WindowsPowerShell\\v1.0\\powershell.exe",
    ...
    DWORD windows_directory_length;
    SIZE_T suffix_index;
    ...
    windows_directory_length = GetWindowsDirectoryW(
        windows_directory,
    ...
    for (suffix_index = 0;
        ...
        path[0] = L'\0';
        if (!append_wide(path, capacity, &length, windows_directory) ||
            !append_wide(path, capacity, &length, suffixes[suffix_index])) {
            continue;
        }
...
void louismod_native_log(const char *message)
{
    ...
    /* Native events are rare, so opening the shared log per event keeps the
     * logger independent of profile changes and process shutdown. */
    if (InterlockedCompareExchange(&g_native_log_guard, 1, 0) != 0) {
    ...
    if (!get_module_directory(
            module_directory,
            sizeof(module_directory) / sizeof(module_directory[0]))) {
        InterlockedExchange(&g_native_log_guard, 0);
        return;
    }
    ...
    if (!append_wide(
        ...
        !append_wide(
            ...
            sizeof(log_path) / sizeof(log_path[0]),
            &length,
            L"\\multibruteforce.log")) {
        InterlockedExchange(&g_native_log_guard, 0);
        return;
    }

    GetLocalTime(&time);
    line_length = wsprintfA(
        line,
        "[%04u-%02u-%02u %02u:%02u:%02u.%03u][native][pid %lu] %s\r\n",
        (unsigned)time.wYear,
...
static void native_log_code(const char *message, DWORD error_code)
{
    char formatted[160];
    int length;
    ...
    if (length > 0 && length < (int)sizeof(formatted)) {
        louismod_native_log(formatted);
    } else {
        louismod_native_log(message);
    }
}
...
static BOOL __attribute__((noinline)) launch_workflow(void *dialog)
{
    ...
    louismod_native_log("native: launch workflow entered");

    if (!get_module_directory(module_directory,
                              sizeof(module_directory) /
                                  sizeof(module_directory[0]))) {
        louismod_native_log("native: could not resolve the DLL directory");
        return FALSE;
    }

    louismod_native_log("native: DLL directory resolved");
    ...
    if (!append_wide(script_path,
        ...
        louismod_native_log("native: launcher path is too long");
        return FALSE;
    }

    if (GetFileAttributesW(script_path) == INVALID_FILE_ATTRIBUTES) {
        native_log_code(
            "native: launcher script was not found",
            GetLastError());
        return FALSE;
    }

    if (!resolve_powershell_path(
            powershell_path,
            sizeof(powershell_path) / sizeof(powershell_path[0]))) {
        louismod_native_log(
            "native: PowerShell executable path was not found");
        return FALSE;
    }

    louismod_native_log(
        "native: PowerShell executable path resolved");
    ...
    if (!append_wide(g_command_line,
        ...
        !append_wide(g_command_line,
                     sizeof(g_command_line) / sizeof(g_command_line[0]),
                     &length, L"\" -ReplayPath \"")) {
        louismod_native_log("native: launcher command is too long");
        return FALSE;
    }
    ...
    if (!append_selected_replay_paths(
        ...
        louismod_native_log(
            "native: could not resolve the selected replay files");
        return FALSE;
    }

    louismod_native_log("native: selected replay files resolved");
    ...
    if (!CreateProcessW(powershell_path, g_command_line, NULL, NULL, FALSE,
                        CREATE_NO_WINDOW, NULL, module_directory,
                        &startup, &process)) {
        native_log_code(
            "native: CreateProcessW failed",
            GetLastError());
        return FALSE;
    }
    ...
    louismod_native_log("native: PowerShell launcher started");
    return TRUE;
}
...
{
    louismod_native_log("native: export button callback entered");

    if (InterlockedCompareExchange(&g_launch_guard, 1, 0) != 0) {
        louismod_native_log("native: export callback ignored by launch guard");
        return;
    }

    if (!launch_workflow(dialog)) {
        InterlockedExchange(&g_launch_guard, 0);
        louismod_native_log("native: launch workflow failed");
        return;
    }

    /* Close this replay dialog after a successful one-shot handoff. */
    on_replay_back(dialog);
    louismod_native_log("native: replay dialog close requested");
}
...
static BOOL install_hooks(void)
{
    ...
    louismod_native_log("native: install hooks entered");

    g_game_base = (BYTE *)GetModuleHandleW(NULL);
    if (g_game_base == NULL) {
        louismod_native_log("native: game module was not available");
        return FALSE;
    }
    ...
    if (!bytes_equal(capture_site, kCaptureOriginal,
        ...
        louismod_native_log(
            "native: TrackMania 2.12.0 replay signatures did not match");
        return FALSE;
    }
    ...
    *(int32_t *)(void *)&back_patch[1] = (int32_t)relative;
    ...
    if (!write_memory(callback_slot, &hook_callback,
        ...
        write_memory(capture_site, kCaptureOriginal,
                     sizeof(kCaptureOriginal));
        louismod_native_log("native: could not install the minimal hooks");
        return FALSE;
    }

    g_installed = TRUE;
    louismod_native_log("native: minimal replay button hooks installed");
    return TRUE;
}
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, LPVOID reserved)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        louismod_native_log("native: DLL process attach");
        install_hooks();
#ifdef LOUISMOD_STANDALONE_RENDERER
